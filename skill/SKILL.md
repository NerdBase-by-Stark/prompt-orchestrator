...
> **CRITICAL DISTINCTION**: A well-structured implementation plan is NOT the same as a
> PM orchestration document. Plans have phases, validation, and dependencies — but PM
> orchestration requires explicit subagent coordination structure. Score ONLY PM-specific
> signals, not general organization quality.

**Scoring Signals:**

| Signal | Points | What Qualifies | What Does NOT Qualify |
|--------|--------|----------------|----------------------|
| PM/Coordinator role definition | 20 | "You are the PM", "coordinate subagents", explicit PM rules | Having phases or numbered steps |
| Task sequence TABLE with agent assignments | 20 | Markdown table with Order/Task/Blocker/Agent columns | Numbered phase list, TODO list |
| Subagent spawn instructions | 15 | Agent tool syntax, spawn prompts, `subagent_type` references | "Implement this", "do step X" |
| Status protocol | 15 | STATUS: COMPLETE/FAILED/BLOCKED protocol, progress tracker | Checkboxes, validation criteria |
| Context isolation rules | 15 | "PM does not read task files", context discipline section | General scope constraints |
| Error handling with PM escalation | 15 | PM-specific halt/report/escalate procedures | Generic error handling |

**Key Distinctions:**

| Source Has | Score Impact | Reasoning |
|------------|-------------|-----------|
| Phases with code changes | 0 pts | This is a plan, not PM orchestration |
| Validation criteria / checkboxes | 0 pts | Plans have validation too — not PM-specific |
| Dependencies between phases | 0 pts | Plans have dependencies too — not PM-specific |
| "Spawn subagent for each task" | 15-20 pts | This IS PM orchestration language |
| `subagent_type: "fullstack-developer"` | 15-20 pts | This IS PM orchestration language |
| PROGRESS TRACKER with Status column | 15 pts | This IS PM coordination structure |
...
**USE AS-IS Execution Path (MANDATORY when USE AS-IS is chosen):**

When the user chooses USE AS-IS, do NOT skip artifact generation. The existing document
becomes the SOURCE for a streamlined extraction. You MUST still generate orchestration artifacts.

**USE AS-IS Procedure:**

1. **Map existing structure to tasks**
   - Each phase/major section → one task file
   - Preserve the original section content verbatim in task files
   - If phases are too large (500+ lines), split per Context Rot rules (Phase 1.7)

2. **Classify each task's action type** (MANDATORY)
   - Use the Action Classification table in `references/capability-inference.md`
   - `implementation` (modifies files), `testing`, `research`, `review`, `devops`, `documentation`
   - This determines which agents are eligible

3. **Run Context Rot Risk Assessment** (Phase 1.7) on each mapped task
   - Populate the Context Rot section in TASK-MANIFEST.md
   - Split oversized tasks (500+ lines) per Phase 1.7 rules before proceeding

4. **Run Agent Discovery** (Phase 1.6) and assign agents
   - Check the Agent Tool Capability Matrix in `references/capability-inference.md`
   - HARD GATE: implementation tasks MUST get write-capable agents

5. **Generate ALL orchestration artifacts:**
   - `PM-ORCHESTRATION.md` — using the template, populated with task sequence from the plan
   - `TASK-MANIFEST.md` — with Action Type and Can Write columns
   - `CONTEXT.md` — shared context extracted from plan preamble/overview
...
If the orchestration directory ends up empty, this is a **skill failure**.
...
Step 3: Cross-reference discovered agents with the Agent Tool Capability Matrix
        in references/capability-inference.md to determine which can Write/Edit
Step 4: Record findings in TASK-MANIFEST.md "Available Agents" section (include Can Write/Edit column)
Step 5: If Step 1 and Step 2 yield nothing, default to general-purpose for all tasks
```

**Matching Rules (abbreviated — see `references/capability-inference.md` for full priority table):**

| Priority | Match Type | Example |
|----------|-----------|---------|
| 0 (gate) | Action-type compatibility check | Task is `implementation` + Agent lacks Write → **REJECT** |
...
| 3-4 | Domain match / Fallback | See `references/capability-inference.md` for full breakdown |

**NEVER assign an agent type that you did not find in Steps 1-2.** If uncertain, default to `general-purpose`.

**HARD GATE**: Before finalizing any agent allocation, verify it passes the action-type compatibility check (Priority 0). Implementation tasks MUST get agents with Write/Edit tools. See `references/capability-inference.md` for the full Agent Tool Capability Matrix and Task Action Classification rules.
